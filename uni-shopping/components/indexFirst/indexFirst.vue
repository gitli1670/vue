<template>
	<view id="main">
		<view class="lun">
			<!-- 轮播 -->
			<view class="swiper">
				<swiper circular autoplay interval="2000" duration="500" @change="onswiper">
					<swiper-item v-for="(item,index) in swiper" :key="index"><image class="img" :src="item.src" /></swiper-item>
				</swiper>
				<view class="ball">
					<view :style="{background:swiperIndex==index?'white':'rgba(0,0,0,.4)'}" v-for="(item,index) in swiper" :key="index"></view>
				</view>
			</view>
			<!-- 弹性布局 -->
			<view class="list">
...
<style scoped>
	...
	.swiper swiper{width: 100%;height: 350upx;}
	.swiper swiper swiper-item{width: 100%;height: 100%;}
	.swiper .ball{height:20upx;position: absolute;display: flex;right: 30upx;bottom: 30upx;}
